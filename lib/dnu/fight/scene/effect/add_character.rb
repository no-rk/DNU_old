    ...
    module Scene
      class AddCharacter < BaseEffect
        ...
        def play_children
          ...
          character_kind        = character_setting[:kind].to_s
          character_name        = character_setting[:name].to_s
          ...
          unless character_unique and @character.find_by_team(対象.team).find_by_kind(character_kind).find_by_name(character_name).find_by_parent_effect(@stack.last).present?
            @character.add_character(character_setting.merge(:team => 対象.team), character_definitions, 対象, @stack.last, true)
            success = true
          end
