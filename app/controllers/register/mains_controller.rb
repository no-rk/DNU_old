class Register::MainsController < Register::ApplicationController
  ...
  def set_instance_variables
    ...
    forget_ids = current_user.result(:art).where(at_arel[:forget].eq(true)).includes(:art_type)
    ...
    blossom_ids = GameData::Art.where(at_arel[:blossom].eq(true)).includes(:art_type).map{|r| r.id }
