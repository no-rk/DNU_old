$ ->
  ...
  $.fn.textEditor = (options = {}) ->
    ...
    this.each ->
      ...
      $color1        = $(this).textEditorButton($('<button>').html("赤").addClass('btn btn-danger'), "文字赤色に。", "赤", "黒")
      $color2        = $(this).textEditorButton($('<button>').html("橙").addClass('btn btn-danger'), "文字橙色に。", "橙", "黒")
      $color3        = $(this).textEditorButton($('<button>').html("黄").addClass('btn btn-danger'), "文字黄色に。", "黄", "黒")
      $color4        = $(this).textEditorButton($('<button>').html("緑").addClass('btn btn-danger'), "文字緑色に。", "緑", "黒")
      $color5        = $(this).textEditorButton($('<button>').html("青").addClass('btn btn-danger'), "文字青色に。", "青", "黒")
      $color6        = $(this).textEditorButton($('<button>').html("藍").addClass('btn btn-danger'), "文字藍色に。", "藍", "黒")
      $color7        = $(this).textEditorButton($('<button>').html("紫").addClass('btn btn-danger'), "文字紫色に。", "紫", "黒")
      ...
      switch options.type
        when 'message', 'document'
          $editor
            .append($icons).append($r_serif).append($s_serif).append($sequence).append($random).append($dice).append($newline)
      switch options.type
        when 'document'
          $editor
